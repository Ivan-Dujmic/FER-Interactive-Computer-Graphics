...
std::vector<std::shared_ptr<Renderable>> ResourceManager::getScene(const std::string &name) {
    ...
    for (std::size_t i = 0 ; i < scene->mNumMeshes ; i++) {
        renderables.emplace_back(std::make_shared<TriangleMesh>(parseMesh(scene->mMeshes[i], scene)));
    }
...
TriangleMesh ResourceManager::parseMesh(aiMesh* mesh, const aiScene* scene) const {
    ...
    if (scene && scene->HasMaterials() && mesh->mMaterialIndex < scene->mNumMaterials) {
        material = parseMaterial(scene->mMaterials[mesh->mMaterialIndex]);
    }
