...
int main(int argc, char * argv[]) {
	...
	if (scene->HasMeshes()) {
		...
		for (int i = 0; i < 10 && i < mesh->mNumFaces; i++) {
			...
			//OPREZ! razmisliti prije nego se kopiraju dijelovi ovog koda. OpenGL �eli indeksirane vrhove, normale i uv koordinate, a tu smo prikazali te podatke slijedno, a ne indeksirano!
	...
	if (scene->HasMaterials()) {
		...
		for (int i = 0; i < scene->mNumMaterials; i++) {
			...
			// emissive komponenta se ne trazi za implementirati u lab. vje�bama
			std::cout << "emissive: ";
